...
function DessertsList(props) {
  const filtered = props.data.filter(value => value.calories < 500 )
  const x = filtered.sort((a, b) =>  a.calories - b.calories )
  const listItems = x.map(value => {
    const text = `${value.name} - ${value.calories} cal`
    return <li>{text}</li>
  })
  return(
  <ul>
